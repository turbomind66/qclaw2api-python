"""pool.py — 账号池：单一状态机（健康/冷却/熔断）+ 在途租约 + 三因子加权挑选 + 持久化。

从 workbuddy2api-python 原样移植，两处语义调整：
  1. Auth 换成 QClaw 的静态 apiKey 凭证（**没有** refresh/过期概念，pool 不再关心 token）
  2. `credits` 字段的语义是「剩余额度点数」（QClaw 的积分/额度），
     挑选时仍然偏好额度高的账号，字段名保持不变以免引入无谓差异。
...
LOG = logging.getLogger("q2api.pool")

# CoolKind
COOL_HARD = 0  # 额度耗尽 → 冷却到次日 04:00
COOL_SOFT = 1  # 429 → 短冷却
