...
从 workbuddy2api-python 原样移植，仅改 key 前缀与 logger 名。
"""
    ...
    def set_bind(self, key: str, uid: str, ttl: timedelta) -> None:
        ...
        def run():
            try:
                ...
                LOG.debug("[redisstore] SetBind %s: %s", key, e)
    ...
    def del_bind(self, key: str) -> None:
        def run():
            try:
                ...
                LOG.debug("[redisstore] DelBind %s: %s", key, e)
        threading.Thread(target=run, daemon=True).start()
    ...
    def save_state(self, data: bytes) -> None:
        def run():
            try:
                ...
                LOG.debug("[redisstore] SaveState: %s", e)
        threading.Thread(target=run, daemon=True).start()
...
def new(url: str, token: str) -> Store:
    ...
    try:
        ...
        LOG.warning("[redisstore] 未安装 redis 库，降级 Noop（纯内存模式）。pip install redis 可启用")
        return Noop()
    ...
    try:
        ...
        LOG.warning("[redisstore] redis 连接串解析失败 (%s)，降级 Noop", e)
        return Noop()
    try:
        if not client.ping():
            LOG.warning("[redisstore] upstash ping 失败，降级 Noop（纯内存模式）")
            client.close()
            return Noop()
    except Exception as e:
        LOG.warning("[redisstore] upstash 连接失败 (%s)，降级 Noop（纯内存模式）", e)
        try:
            client.close()
        except Exception:
            pass
